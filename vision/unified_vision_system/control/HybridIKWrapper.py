...
This module provides a convenient interface for integrating UR30 kinematics with the
existing VLM (Vision-Language Model) robotic control pipeline.
...
from unified_vision_system.control.UR30Kinematics import (
...
class VLMKinematicsController:
    ...
    def __init__(self,
        ...
        if ik_solver is not None:
            ...
            # UR30 currently uses numerical/RTB solvers; ikfast is unavailable.
            base_solver = HybridUR30Kinematics(
                enable_fallback=True,
                debug=debug
            )
            self.ik_solver = base_solver
        ...
        if debug:
            ...
            print(f"ur_ikfast available: {getattr(self.ik_solver, 'ikfast_available', False)}")
    ...
    def solve_for_vlm_target(self,
        ...
        if getattr(self.ik_solver, 'ikfast_available', False) and solutions:
