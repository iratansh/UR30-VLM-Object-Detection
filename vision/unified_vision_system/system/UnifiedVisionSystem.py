...
The system enables natural language control of a UR30 robot
...
from unified_vision_system.perception.DepthAwareDetector import DepthAwareDetector, Detection3D
...
from unified_vision_system.calibration.HandEyeCalibrator import HandEyeCalibrator
import pyrealsense2 as rs
import threading

# Default constants, can be overridden by ROS parameters
DEFAULT_ROBOT_NAMESPACE = 'ur30'
...
class UnifiedVisionSystem(Node):
    ...
    None : The class uses ROS2 parameters for configuration
        - robot_namespace : Name of the robot (default: 'ur30')
    ...
    def __init__(self):
        ...
        try:
            ...
            if enable_hybrid_ik:
                self.logger.info("Initializing UR30 IK system with adaptive controller")
                self.kinematics = UR30Kinematics(debug=ik_debug)
                self.ik_controller = VLMKinematicsController(
                    enable_ikfast=False,
                    adaptive_timeout=True,
                    debug=ik_debug,
                    ik_solver=self.kinematics
                )
            else:
                self.logger.info("Initializing UR30 numerical IK system")
                self.kinematics = UR30Kinematics(debug=ik_debug)
    ...
    def _check_joint_limits(self, joint_angles: List[float]) -> bool:
        ...
        """
        # UR30 joint limits in radians (matching UR30Kinematics)
        JOINT_LIMITS = [
            (-2 * np.pi, 2 * np.pi),
            (-np.pi, np.pi),
            (-np.pi, np.pi),
    ...
    def _format_and_publish_ros2_command(self, joint_angles: List[float]):
        ...
        Uses JointTrajectory message format required by UR30 ROS2 driver
        """
