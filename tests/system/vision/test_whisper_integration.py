...
def test_whisper_integration():
    ...
    try:
        ...
        logger.info("PASS Whisper model loaded successfully")
        
        # Test audio device availability
        devices = sd.query_devices()
        logger.info(f"PASS Found {len(devices)} audio devices")
        
        # Test SpeechCommandProcessor import
        from unified_vision_system.hri.SpeechCommandProcessor import SpeechCommandProcessor
        logger.info("PASS SpeechCommandProcessor imported successfully")
        ...
        logger.info("PASS SpeechCommandProcessor initialized successfully")
        ...
        for cmd in test_commands:
            ...
            if result:
                logger.info(f"PASS Command '{cmd}' -> Intent: {result['intent']}, Target: {result['target']}")
            else:
                logger.warning(f"WARNING Command '{cmd}' could not be processed")
        
        # Clean up
        processor.cleanup()
        logger.info("PASS Phase 1A Whisper integration test completed successfully!")
        return True
